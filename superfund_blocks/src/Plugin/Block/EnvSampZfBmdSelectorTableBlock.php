...
/**
 * Provides an Environmental Sample Zebrafish BMD Endpoint Selector Table block.
 *
 * Lists zebrafish endpoints for a sample (grouped into Morphological and
 * Behavioral) with AUC/BMD10/BMD50 summary bars. Clicking an endpoint that
 * has data calls into the companion EnvSampZfBmdRespChartBlock's exposed
 * render function — window.superfundBlocks.envSampZfBmdResp['<chart_id>'] —
 ...
 * sample id, so they only need to be placed on the same sample page.
 *
 * @Block(
 *   id = "superfund_blocks_env_samp_zf_bmd_selector_table",
 *   admin_label = @Translation("Environmental Sample Zebrafish BMD Endpoint Selector Table"),
 *   category = @Translation("Superfund Blocks"),
 * )
 */
class EnvSampZfBmdSelectorTableBlock extends BlockBase implements BlockPluginInterface, ContainerFactoryPluginInterface {
  ...
  public function build(): array {
    ...
    // -------------------------------------------------------------------------
    // 3. Fetch every known endpoint, joined against this sample's data
    //    (so endpoints with no data for this sample still list, unlinked),
    ...
    $sql = "SELECT
      ...
      LEFT JOIN view_zebrafishSampDoseResponse zsdr
        ON zsdr.End_Point_Name = cen.End_Point_Name AND zsdr.Sample_ID = :sample_id
...
(function init() {
  ...
  table.addEventListener('click', function (event) {
    ...
    if (window.superfundBlocks) {
      if (window.superfundBlocks.envSampZfBmdResp) {
        render = window.superfundBlocks.envSampZfBmdResp['{$chart_id}'];
      }
    }
      ...
      '#attached' => [
        'html_head' => [
          [
            ...
            'superfund_env_samp_zf_bmd_selector_init_' . $sanitized_id,
          ],
        ],
      ],
      '#cache' => [
