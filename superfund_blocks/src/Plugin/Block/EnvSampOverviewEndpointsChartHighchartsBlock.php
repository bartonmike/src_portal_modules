...
/**
 * Provides an Environmental Samples Overview Endpoints chart block
 * (Highcharts variant).
 *
 * Displays a Highcharts horizontal bar chart of how many samples have
 * zebrafish XY-coordinate data for each endpoint (site-wide, not scoped to
 * a single sample). Same query and data as EnvSampOverviewEndpointsChartBlock,
 * rendered with Highcharts instead of Plotly.
 *
 * @Block(
 *   id = "superfund_blocks_env_samp_overview_endpoints_chart_highcharts",
 *   admin_label = @Translation("Environmental Samples Overview Endpoints Chart (Highcharts)"),
 *   category = @Translation("Superfund Blocks"),
 * )
 */
class EnvSampOverviewEndpointsChartHighchartsBlock extends BlockBase implements BlockPluginInterface, ContainerFactoryPluginInterface {
  ...
  public function build(): array {
    // -------------------------------------------------------------------------
    // 1. Fetch the number of samples with XY-coordinate data per endpoint,
    //    across all samples.
    // -------------------------------------------------------------------------
    $sql = "SELECT DISTINCT
        s.End_Point_Name AS category,
        (SELECT COUNT(s2.Sample_ID)
         FROM view_zebrafishSampXYCoords s2
         WHERE s2.End_Point_Name = s.End_Point_Name) AS value
      FROM view_sample_endpoints s
      WHERE s.End_Point_Name IS NOT NULL";
    ...
    $chart_id = 'chart-env-samp-overview-endpoints-highcharts';
    ...
    // -------------------------------------------------------------------------
    $descriptor = "<div class='env-samp-overview-endpoints-highcharts element-descriptor'>"
      . "<strong>Endpoints Measured:</strong> Many samples were tested in our zebrafish "
...
(function init() {
  ...
  var settings = drupalSettings.superfundBlocks.envSampOverviewEndpointsHighcharts['{$chart_id}'];
  ...
  var chartOptions = {
    ...
    exporting: {
      filename: 'env-samp-overview-endpoints',
      csv: {
...
JS;
    ...
    return [
      ...
      '#attached' => [
        ...
        'drupalSettings' => [
          'superfundBlocks' => [
            'envSampOverviewEndpointsHighcharts' => [
              $chart_id => [
        ...
        'html_head' => [
          ...
          [
            ...
            'superfund_env_samp_overview_endpoints_highcharts_init',
          ],
        ],
      ],
      '#cache' => [
