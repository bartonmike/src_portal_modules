...
/**
 * Provides an Environmental Sample Extract Molar Concentration chart block.
 *
 * Displays a Plotly bar chart of extract molar concentrations for every
 * chemical detected in a sample identified by the ?id= query parameter.
 * Bars and x-axis labels link through to the chemical detail page.
 *
 * @Block(
 *   id = "superfund_blocks_env_samp_ext_mol_conc_chart",
 *   admin_label = @Translation("Environmental Sample Extract Molar Concentration Chart"),
 *   category = @Translation("Superfund Blocks"),
 * )
 */
class EnvSampExtMolConcChartBlock extends BlockBase implements BlockPluginInterface, ContainerFactoryPluginInterface {
  ...
  public function build(): array {
    ...
    $sql = "SELECT DISTINCT
      ...
      WHERE
        vstc.measurement_value_molar IS NOT NULL
        AND vstc.measurement_value_molar != 0
    ...
    $chart_id = 'chart-samp-ext-mol-conc-' . $sanitized_id;
    ...
    //      DOMContentLoaded.
    // -------------------------------------------------------------------------
    $descriptor = "<div class='inst-moles-meas-plot element-descriptor'>"
...
(function init() {
  ...
  var settings      = drupalSettings.superfundBlocks.envSampExtMolConc['{$chart_id}'];
  ...
  function downloadCsv() {
    var headers = ['Chemical Name', 'Chemical ID', 'Molar Concentration', 'Unit'];
    ...
    a.href     = url;
    a.download = 'env-samp-ext-mol-conc-{$sanitized_id}.csv';
    a.click();
      ...
      '#attached' => [
        ...
        'html_head' => [
          ...
          [
            ...
            'superfund_env_samp_ext_mol_conc_init_' . $sanitized_id,
          ],
        ],
      ],
      '#cache' => [
